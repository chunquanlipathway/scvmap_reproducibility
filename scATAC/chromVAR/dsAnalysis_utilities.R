...
read_mtx_scATACpro2 <- function(mtx_path,GSMID){
  #mtx_path <- paste0(dirt, "matrix.mtx")
  mtx.dir = dirname(mtx_path)
  feature_path <- paste0(mtx.dir, "/",GSMID,"_features.txt")
  barcode_path <- paste0(mtx.dir, "/",GSMID,"_barcodes.txt")
...
do_DA_motif <- function(mtx_score, clusters, test = 'wilcox',
  ...
  for(cluster0 in cls){
    ...
    mtx1 = as.matrix(mtx_score[, colnames(mtx_score) %in% bc0])
    mtx2 = mtx_score[, !colnames(mtx_score) %in% bc0]
	if(length(bc0)!=1){
    mu1 = sapply(1:length(features), function(x) mean(mtx1[x, ]))}else{mu1=mtx1}
